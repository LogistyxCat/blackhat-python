...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if receive_first:
        ...
        # send it to out response handler
    ...
    #now lets loop and read from local, send to remote, send to local, ad infi
    while True:
...
def receive_from(connection):
    ...
    # depending on your target, this may need adjusting
    connection.settimeout(2)

    try:
        # keep reading into the buffer until
        # there's no more data or timeout
        while True:
...
# modify response destined for local host
